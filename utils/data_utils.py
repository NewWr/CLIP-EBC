...
def get_ukb_dataloader(args: ArgumentParser, split: str = "train", ddp: bool = False) -> Union[Tuple[DataLoader, Union[DistributedSampler, None]], DataLoader]:
    ...
    dataset = datasets.UKBDataset(
        ...
        return_filename=False,
        num_crops=args.num_crops if split == "train" else 1,
        # 关键：分层分箱数与 truncation 对齐；若无，则使用 --stratify_bins；再无则回退到 15
        stratify_bins=(args.truncation if hasattr(args, 'truncation') and args.truncation
                       else (args.stratify_bins if hasattr(args, 'stratify_bins') and args.stratify_bins
                             else 15)),
        # 固定随机种子，保证每次运行划分一致
