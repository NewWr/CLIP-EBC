...
parser.add_argument("--model", type=str, default="clip_convnext_b", help="The model to train.")
...
parser.add_argument("--batch_size", type=int, default=48, help="The training batch size.")
parser.add_argument("--num_crops", type=int, default=2, help="The number of crops for multi-crop training.")
parser.add_argument("--min_scale", type=float, default=0.6, help="The minimum scale for random scale augmentation.")
parser.add_argument("--max_scale", type=float, default=1.0, help="The maximum scale for random scale augmentation.")
parser.add_argument("--brightness", type=float, default=0.2, help="The brightness factor for random color jitter augmentation.")
...
parser.add_argument("--T_0", type=int, default=10, help="Number of epochs for the first restart.")
...
parser.add_argument("--total_epochs", type=int, default=200, help="Number of epochs to train.")
parser.add_argument("--eval_start", type=int, default=1, help="Start to evaluate after this number of epochs.")
...
parser.add_argument("--save_best_k", type=int, default=1, help="Save the best k checkpoints.")
parser.add_argument("--save_only_best", action="store_true", help="Only save best checkpoints, skip regular checkpoint saving.")
...
parser.add_argument('--data_path', default='/opt/DM/OCT/StandardData/preprocessed_images/', type=str,
                        help='dataset path')
parser.add_argument('--excel_path', default='/opt/DM/OCT/StandardData/metadata/ukb_train_val_set.xlsx', type=str,
                        help='path to Excel metadata file')
parser.add_argument('--target_column', default='hba1c', type=str,
...
def run(local_rank: int, nprocs: int, args: ArgumentParser) -> None:
    ...
    if args.regression:
        ...
        if args.dataset_type == 'ukb':
            dataset_key = args.target_column  # 使用 target_column 作为键
        else:
            dataset_key = args.dataset  # 使用 dataset 作为键
    ...
    model = get_model(
        ...
        # === new: prompt/phrase related hyperparameters ===
        indicator_name=args.target_column,           # auto-match different variable names
        indicator_unit=args.indicator_unit,
        phrase_config_path=args.phrase_config,
        prompt_template=args.prompt_template,
        name_syns_mode=args.name_syns_mode,
        phrase_pick_mode=args.phrase_pick_mode,
        phrase_seed=args.phrase_seed,
    ).to(device)
    ...
    for epoch in range(start_epoch, args.total_epochs + 1):  # start from 1
        ...
        if local_rank == 0:
            ...
            # 只有在不是"仅保存最佳"模式时才进行定期保存
            if not args.save_only_best and (epoch % args.save_freq == 0):
                save_checkpoint(
                    epoch + 1,
...
# Prompt & phrase configuration (new)
parser.add_argument("--phrase_config", type=str,
                    default="/opt/DM/OCT/CLIP_Code/CLIP-EBC/configs/hba1c_bin_phrases_15.json",
                    help="Path to phrase JSON with pro/plain name syns and bin label phrases.")
# parser.add_argument("--prompt_template", type=str,
#                     default="A fundus image of {name} {rng} suggests {label}.",
#                     help="Template for forming text prompts. Placeholders: {name}, {rng}, {label}.")
parser.add_argument("--prompt_template", type=str,
                    default="a photo of {name} {rng} suggests {label}.",
                    help="Template for forming text prompts. Placeholders: {name}, {rng}, {label}.")
parser.add_argument("--name_syns_mode", type=str, default="pro",
                    choices=["pro", "plain", "random"],
                    help="Which name synonym pool to use when forming prompts.")
parser.add_argument("--phrase_pick_mode", type=str, default="random",
                    choices=["random", "first"],
                    help="How to pick phrases from each bin phrase list.")
parser.add_argument("--phrase_seed", type=int, default=None,
                    help="Random seed for phrase sampling (None means global RNG).")
parser.add_argument("--indicator_unit", type=str, default="",
                    help="Unit string used when formatting ranges, e.g., 'mmHg' for BP.")
parser.add_argument("--stratify_bins", type=int, default=15,
                    help="Number of bins for stratified sampling in UKBDataset. "
                         "Defaults to truncation when classification; for regression, set explicitly or fallback to 15.")
