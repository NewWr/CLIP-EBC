...
def get_target_layer(model, backbone):
    ...
    if "resnet" in backbone:
        ...
        # 对于 CLIP-EBC 的 ConvNeXt：直接挂在模型内部的 image_encoder（权重来自 checkpoint）
        # image_encoder 是 models/clip/_clip/image_encoder.py 中的 ConvNeXtEncoder 包装器，
        # forward 返回 features_only 的目标 stage 特征 [N, C, H, W]，非常适合 CAM。
        if hasattr(model, "image_encoder"):
            return model.image_encoder
        # 兜底逻辑（不建议走到这里）：尝试抓取最后一个 stage 的 block
        enc = getattr(model, "image_encoder", None) or getattr(model, "backbone", None)
        if enc is not None and hasattr(enc, "backbone") and hasattr(enc.backbone, "stages"):
            try:
                return enc.backbone.stages[-1].blocks[-1]
            except Exception:
                return enc.backbone.stages[-1]
        # 最后的保底
...
def main():
    ...
    # Target parameters
    parser.add_argument('--target_column', default='sysbp', type=str,
                        choices=['sysbp', 'sysbp', 'hba1c', 'fbg', 'ldl', 'hdl'],
...
# Example usage:
# /opt/miniconda/envs/py310/bin/python /opt/DM/OCT/CLIP_Code/CLIP-EBC/visualize_ukb.py --model clip_convnext_b --target_column sysbp --weight_path /opt/DM/OCT/CLIP_Code/CLIP-EBC/checkpoints/ukb/clip_convnext_b_word_224_8_15_fine_1.0_mae/best_mae_0.pth --image_path /opt/DM/OCT/CLIP_Code/CLIP-EBC/visualization/5050591_21013_0_0.png --methods gradcam layercam gradcam++ --device cpu