...
# 模块级新增类与函数：ConvNeXtBlock 和 make_convnext_layers

class ConvNeXtBlock(nn.Module):
    """
    A minimal ConvNeXt-style block for decoder:
    - Depthwise 7x7 Conv
    - LayerNorm in channels-last (NHWC)
    - Pointwise MLP: Linear(C -> 4C) -> GELU -> Linear(4C -> C')
    - Residual connection (with 1x1 conv if C != C')
    """
    def __init__(
        self,
        in_channels: int,
        ...
        self.pw1 = nn.Linear(in_channels, 4 * in_channels)
        self.act = nn.GELU()
        self.pw2 = nn.Linear(4 * in_channels, out_channels)

        # 可学习的层缩放（与 ConvNeXt 一致的稳定技巧）
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = self.dwconv(x)                         # N, C, H, W
        y = y.permute(0, 2, 3, 1)                  # N, H, W, C
        y = self.norm(y)
        y = self.pw1(y)
        y = self.act(y)
        y = self.pw2(y)                            # N, H, W, C'
...
def make_convnext_layers(
    block: nn.Module,
    cfg: List[Union[int, str]],
    in_channels: int,
) -> nn.Sequential:
    """
    构建 ConvNeXt 风格的解码层序列：
    - cfg 元素为 int 时，追加一个 ConvNeXtBlock(in_channels -> v)
    - cfg 元素为 "U" 时，追加一个上采样层 Upsample(scale_factor=2, bilinear)
    """
    ...
    for v in cfg:
        if v == "U":
            ...
            layers.append(block(in_channels=in_channels, out_channels=v))
            in_channels = v
    seq = nn.Sequential(*layers)
    seq.apply(_init_weights)
    return seq
