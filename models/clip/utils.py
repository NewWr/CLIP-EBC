...
def format_blood_indicator(
    ...
    为血液指标生成文本提示
    Args:
        indicator_ranges: 血液指标范围列表 [(low1, high1), (low2, high2), ...]
        indicator_name: 指标名称，如 "glucose", "cholesterol"
        unit: 单位，如 "mg/dL", "mmol/L"
        prompt_type: 提示类型 "word" 或 "number"
    Returns:
        文本提示列表
    """
    prompts = []
    ...
    for i, (low, high) in enumerate(indicator_ranges):
        if i == 0:  # 正常范围
            if low == high:
                prompt = f"The {indicator_name} level is {low}{unit_str}."
            else:
                prompt = f"The {indicator_name} level is between {low} and {high}{unit_str}."
        else:  # 异常范围
            if low == high:
                prompt = f"The {indicator_name} level is {low}{unit_str}."
            elif low == float('inf') or high == float('inf'):
                prompt = f"The {indicator_name} level is extremely high (>{low}{unit_str})."
            elif low == float('-inf') or high == float('-inf'):
                prompt = f"The {indicator_name} level is extremely low (<{high}{unit_str})."
            else:
                prompt = f"The {indicator_name} level is between {low} and {high}{unit_str}."
