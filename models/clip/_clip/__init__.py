...
from .image_encoder import ModifiedResNet, VisionTransformer, ConvNeXtEncoder
...
# 文件顶部 import 增加 ConvNeXtEncoder
def _convnext(
    name: str,
    features_only: bool = False,
    out_indices: Optional[Tuple[int, ...]] = None,
    ...
    **kwargs: Any,
    ...
    # 从配置读取 variant/embed_dim/image_resolution
    variant = config.get("variant", "convnext_base")
    output_dim = int(config.get("embed_dim", 512))
    image_resolution = int(config.get("image_resolution", 224))

    # 检查本地权重是否存在，决定是否使用预训练权重
    weight_path = os.path.join(curr_dir, "weights", f"clip_image_encoder_{name}.pth")
    use_pretrained = not os.path.exists(weight_path)
    ...
    model = ConvNeXtEncoder(
        ...
        reduction=reduction,
        out_indices=out_indices,
        pretrained=use_pretrained,  # 只有在没有本地权重时才使用预训练权重
    )

    # 若存在本地权重则加载
    if os.path.exists(weight_path):
        print(f"[INFO] Loading local weights from {weight_path}")
        ...
        print(f"[INFO] No local weights found for {name}, using timm pretrained weights.")

    return model

# 向外暴露 convnext 的 img/txt
def convnext_b_img(features_only: bool = False, out_indices: Optional[Tuple[int, ...]] = None, **kwargs: Any) -> ConvNeXtEncoder:
    return _convnext("convnext_b", features_only=features_only, out_indices=out_indices, **kwargs)

def convnext_b_txt() -> CLIPTextEncoder:
    return _text_encoder("convnext_b")
